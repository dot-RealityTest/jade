...
struct MainWindow: View {
    ...
    private var commandPaletteAppItems: [CommandPaletteItem] {
        MainWindowCommandPaletteBuilder.appItems(context: commandPaletteContext)
    }

    private var commandPaletteContext: MainWindowCommandPaletteContext {
        MainWindowCommandPaletteContext(
            snippetsScopeMode: snippetsScopePreferences.mode,
            activeProjectName: activeProject?.name,
            ollamaModel: NaturalCommandSettings.shared.ollamaModel
        )
    }
    ...
    private func initializeJourney() {
        ...
        do {
            try MainWindowJourneyActions.initialize(project: project, worktreePath: path)
    ...
    private func presentJourneyNextStep() {
        ...
        let path = activeWorktreePath(for: project)
        do {
            let proposal = try MainWindowJourneyActions.loadNextStepProposal(projectPath: path)
    ...
    private func confirmJourneyStep(_ proposal: JourneyStepProposal, project: Project, overrideBlocker: Bool) {
        ...
        let prefill = MainWindowJourneyActions.richInputPrefill(for: proposal)
    ...
    private func completeJourneyStep() {
        ...
        let path = activeWorktreePath(for: project)
        do {
            let proposal = try MainWindowJourneyActions.completeStep(project: project, worktreePath: path)
            logJourneySession(
                outcome: .completed,
                proposal: proposal,
                project: project,
