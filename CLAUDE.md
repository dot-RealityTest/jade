...
- Snippets use **general** (shared) vs **project** scopes (palette or **⌘⌃J**); a **remote space always shows its own snippets** regardless of mode; do not add chrome icons for snippet scope mode.
...
- Obsidian MCP was removed (2026-06-12): logs/captures write markdown directly into a user-chosen logs folder (Settings → Logs & Capture); do not reintroduce MCP/Python server integration unless the user asks. Study open-source patterns only (Warp AGPL — no code copy).
