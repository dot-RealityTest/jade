...
Muxy/
  ...
  Models/
    ...
    WorktreeConfig.swift      Decoder for .jade/worktree.json setup commands (.muxy/ legacy fallback)
  ...
  Services/
    ...
    LocalPortMonitor.swift    lsof-backed local TCP listener snapshot, active list, and session dead-port tracking
    ...
    RemoteSpacesStore.swift   @Observable remote machine spaces store, persisted to remote-spaces.json, including legacy SSH command migration
    ...
    WorktreeSetupRunner.swift Dispatches .jade/worktree.json setup commands to a new tab
...
- **Upgrade Homebrew**: Cmd+K exposes `Upgrade Homebrew`, opening a named command tab that runs `brew update && brew upgrade` when Homebrew is installed.
- **Ollama commands**: Cmd+K exposes `Ollama List Models`, `Ollama Pull Model`, `Ollama Run Model`, and `Ollama Serve`. Pull and run use the model from Settings → Natural Commands (default `llama3.2` when unset). `LocalShellCommand` prepends Homebrew paths for GUI-spawned shells and keeps one-shot commands (`list`, `pull`, brew upgrade) in an interactive shell after they finish.
- **App icon**: Master artwork lives at `Muxy/Resources/AppIcon.png`. Run `scripts/update-app-icon.sh [path]` to regenerate `AppIcon.appiconset` sizes and rebuild the SPM resource bundle; `scripts/strip-icon-black-matte.py` removes outer black letterboxing so only the rounded icon remains. Local debug launches via `scripts/run-jade.sh` embed `Contents/Resources/AppIcon.icns` (Finder/Dock) and sync fresh `AppIcon.png` into `Muxy_Muxy.bundle`. `MuxyApp.setAppIcon()` prefers the bundled `.icns` over the PNG fallback.
- **App Identity**: The distributed macOS app is named Jade (`CFBundleName` / `CFBundleDisplayName` / `CFBundleExecutable`). The SwiftPM executable target remains `Muxy` for build and test compatibility, then release packaging installs it into the app bundle as `Jade`.
- **Persistence**: All files in `~/Library/Application Support/Muxy/`. Shared directory helper: `MuxyFileStorage`. Shortcuts are stored in `keybindings.json`; custom command shortcuts are stored in `command-shortcuts.json`; general snippets are stored in `snippets.json`; project-specific snippets are stored in `project-snippets/{projectID}.json`; project inspector notes/todos are stored in `project-inspector/{projectID}.json`; remote machine spaces are stored in `remote-spaces.json`; remote-space snippets are stored in `remote-spaces/{slug}/snippets.json`. Snippet scope mode (general vs project) is stored in UserDefaults (`muxy.general.snippetsScopeMode`). Remote spaces are backward-compatible JSON records: simple `ssh user@host` command records are normalized into structured profile fields on load/save, while unsupported custom commands still run through the legacy command field. Remote spaces also persist an optional theme name; blank theme names use name-based defaults for Zen and Alienware. Snippets are backward-compatible JSON records; shared snippets seed common local workflows when storage is missing or empty, while remote scopes seed Linux starter workflows only when their snippet file does not exist yet. Cmd+K remote commands and remote-space snippets open named command tabs through `RemoteCommandBuilder`, which wraps the selected Linux command in the active space's SSH profile instead of running it in the local backing directory; reboot and power-off require an inline second Enter before execution. Natural-language shell commands are not saved as history; settings live in `UserDefaults`, generated commands must be reviewed before running, and useful results can be explicitly saved into the active snippet scope. Worktrees are persisted per-project at `worktrees/{projectID}.json`, including whether a secondary worktree is Muxy-managed or externally discovered. Git projects can manually refresh this list from `git worktree list --porcelain` to import existing worktrees without deleting absent entries; paths are matched after symlink resolution so a repo opened via a symlinked path still collapses onto a single primary entry. Externally discovered worktrees are never touched by Muxy's `cleanupOnDisk` paths (project removal, post-merge cleanup, manual removal) — they can only be unregistered by the user in the underlying repo. Worktree setup commands live in-repo at `{Project.path}/.jade/worktree.json` (a legacy `.muxy/worktree.json` is still read as a fallback).
